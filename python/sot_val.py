...
def calculate_time_score(time_in_ms):
    ...
    elif time_in_ms > 3 and time_in_ms <= 6:
        return 60 + 30 * (6 - time_in_ms) / 3
...
def main():
    input_folder = r"G:\dataset\dolphin_competion\pre\track-train\01"  # 测试图片文件夹
    gt_folder = r"G:\dataset\dolphin_competion\pre\track-train\02"  # 真值标签的txt文件夹路径
    ...
    # print(f"Input folder contains {input_file_count} images.")
    ...
    if count > 0:
        ...
        final_scores = {
            ...
            "average_acc_score": avg_acc_score,
            "average_score": avg_score,
            "total_images_processed": count
        }
        with open(result_log, "w") as log_file:
            json.dump({"log_entries": log_entries, "final_scores": final_scores}, log_file, indent=4)
    else:
